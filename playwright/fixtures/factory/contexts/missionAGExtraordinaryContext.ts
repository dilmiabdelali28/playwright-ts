...
export const missionAGExtraordinaryContext = (buildingName: string) => {
  ...
  const coOwnerVpc = Customer.bob().with({
    firstName: "A-Vpc",
    fullname: "A-Vpc Playwright",
  });
  ...
  const appartements = Array.from({ length: 5 }, (_, index) =>
    Unit.anAppartment()
      .with({
  ...
  const coOwnerAccounts = [
    CoOwnerAccount.anAccount()
      .withBuilding(building)
      .withFullOwnershipHolder(coOwnerVpc)
