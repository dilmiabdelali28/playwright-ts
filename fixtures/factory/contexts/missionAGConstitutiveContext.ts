...
export const missionAGConstitutiveContext = (buildingName: string) => {
  ...
  const missionOrigin = {
    kind: "MissionNewBuildingCreation",
    target: missionId,
  };
  const promoter = Customer.promoter().withOrigin(missionOrigin);
  ...
  const coOwnershipMandate = CoOwnershipMandate.anAGCoOwnershipMandate()
    ...
    .withAgency(agency)
    .withBranch(branch)
    .withStatus("TEMPORARY")
    .withCurrentCreatedAtAndUpdatedAtDates()
    .withMainAssociate(associateId, associates.guilhem._id);

  const bob = Customer.bob();
  const bobAppartement = Unit.anAppartment1ForAGC()
    .withCurrentCreatedAtAndUpdatedAtDates()
    .ofBuilding(building)
    .withDelivery(true);

  const mary = Customer.mary();
  const maryAppartement = Unit.anAppartment2ForAGC()
    .withCurrentCreatedAtAndUpdatedAtDates()
    .ofBuilding(building)
    .withDelivery(true);
  ...
  let openingDate = moment().subtract(1, "years").startOf("day");
  let closingDate = moment().subtract(1, "day").endOf("day");
  ...
  const timeTable = Timetable.aTimetable().withAmount(EUR(1500));
  ...
  const missionTrusteeContract =
    MissionTrusteeContract.aMissionTrusteeContract()
      ...
      .withPropositions([
        {
          _id: propositionId,
          generalAssemblyDate: moment().add(2, "months").toDate(),
