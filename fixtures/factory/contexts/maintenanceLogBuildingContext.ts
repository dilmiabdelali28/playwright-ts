...
/**
 * A co-ownership (AG) building rich enough to exercise the maintenance log:
 * - an ACTIVE co-ownership mandate (the "Carnet d'entretien" page),
 * - allocation keys + validated budgets (voted works / mission repairs),
 * - contracts (the "exclude from maintenance log" toggles).
 *
 * Replaces the hardcoded seeded "Toulouse" building so the test is portable
 * across environments (dev / RA).
 */
export const maintenanceLogBuildingContext = () => {
  ...
  const building = Building.forAG(`${Date.now()} - Immeuble fixture`)
    .with({
  ...
  bobAppartement.withCoOwnerAccount(coOwnerAccounts[0]);
  maryAppartement.withCoOwnerAccount(coOwnerAccounts[1]);
  ...
  const accountingPeriods = Array.from({ length: 3 }, (_, index) => {
  ...
  const contracts = [
    Contract.aContract()
      ...
      .withAmount(EUR(100))
      .withBuilding(building)
      .withLabel("CONTRAT CARNET ENTRETIEN 1")
      .withFonciaContractNumber(`MLOG-${Date.now()}-1`)
      .withStartingDate(today.toDate())
      .withEndingDate(moment(today).add(1, "year").toDate())
      .addCodification({
        allocationKey: allocationKeys[0],
        expenseType,
        rate: 1,
      }),
    Contract.aContract()
      .withSupplier(Supplier.constatimmo())
      .withAgency(agency)
      .withAmount(EUR(200))
      .withBuilding(building)
      .withLabel("CONTRAT CARNET ENTRETIEN 2")
      .withFonciaContractNumber(`MLOG-${Date.now()}-2`)
      .withStartingDate(today.toDate())
      .withEndingDate(moment(today).add(1, "year").toDate())
      .addCodification({
        allocationKey: allocationKeys[1],
        expenseType,
        rate: 1,
      }),
  ];
